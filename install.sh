...
echo -e "${COLOR}"
echo ""
echo "  ______   ______     ______     ______     ______      ______   ______     ______     __        ";      
echo " /\__  _\ /\  __ \   /\  __ \   /\  ___\   /\__  _\    /\__  _\ /\  __ \   /\  __ \   /\ \       ";
echo " \/_/\ \/ \ \ \/\ \  \ \  __ \  \ \___  \  \/_/\ \/    \/_/\ \/ \ \ \/\ \  \ \ \/\ \  \ \ \____  ";
...
if [[ $choice =~ ^[1-2]+$ ]]; then
    ...
    if curl -s -m 10 https://www.google.com > /dev/null || curl -s -m 10 https://www.github.com > /dev/null; then
        ...
        echo "";
        echo -e "${YELLOW}[✔] Downloading toasttool...${NC}"
        if sudo git clone https://github.com/ToastyOfficial/toasttool.git $install_dir; then
...
#            echo '#!/bin/bash' > toasttool.sh
            echo '#!/bin/bash' > $install_dir/toasttool.sh
            echo "source $install_dir/venv/bin/activate" >> $install_dir/toasttool.sh
            echo "python3 $install_dir/toasttool.py \$@" >> $install_dir/toasttool.sh
            chmod +x $install_dir/toasttool.sh
            sudo mv $install_dir/toasttool.sh $bin_dir/toasttool
            echo -e "${GREEN}[✔] Script created successfully [✔]"
        else
            echo -e "${RED}[✘] Failed to download Toast Tool [✘]"
            exit 1
        fi
    ...
    if [ -d $install_dir ]; then
        ...
        echo -e  "${ORANGE}[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
        echo     "[+]                                                             [+]"
        echo -e  "${ORANGE}[+]     ✔✔✔ Now Just Type In Terminal (toasttool) ✔✔✔      [+]"
        echo     "[+]                                                             [+]"
        echo -e  "${ORANGE}[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
    else
